...
3. **CRSP column names** -- `crsp.dsi` uses `date` (not `caldt`), `spindx` (not `sprindx`). v2 daily uses `dly` prefix (`dlycaldt`, `dlyret`, `dlyprc`); monthly uses `mth` prefix (`mthcaldt`, `mthret`). Do NOT use v1 names (`date`, `ret`, `prc`) with v2 tables -- they produce hard errors.
...
8. **CCM linking filters** -- always filter: `linktype IN ('LC','LU') AND linkprim IN ('P','C')` and check date overlap.
9. **CCM link column name** -- the PERMNO column in `crsp.ccmxpf_lnkhist` is `lpermno`, not `permno`. Always alias: `l.lpermno AS permno`.
10. **CCM active links** -- `linkenddt` is NULL for currently active links. Always use `COALESCE(linkenddt, '9999-12-31')`.
11. **Compustat standard filters** -- every `comp.funda`/`comp.fundq` query MUST include: `indfmt='INDL' AND datafmt='STD' AND popsrc='D' AND consol='C'`. Omitting these produces duplicate rows.
12. **First-call timeout** -- the first `psql` call in a session often triggers DUO 2FA and hangs 30-60s. Do not kill it -- accept the push. Retry once before diagnosing.
13. **CRSP `nameendt` spelling** -- `crsp.msenames` uses `nameendt` (single d), NOT `nameenddt`. Compare with `linkenddt` in `ccmxpf_lnkhist` which IS double-d.
14. **WRDS data lag** -- CRSP, Compustat, and other WRDS databases lag months behind the current date. Never use the current year in smoke tests or data validation. Use a historical year (e.g., 2022) guaranteed to be fully populated.
