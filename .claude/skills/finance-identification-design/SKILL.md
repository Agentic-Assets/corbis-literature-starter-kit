...
- `literature_search` (Enterprise; fallback: multiple `search_papers` queries) (topic: the identification challenge, `maxSearches: 2`) → broader sweep for methodological precedents.
- `internet_search` (Enterprise; fallback: `search_papers` for academic content, or ask user for URLs) (query: "site:nber.org [method] [topic]") → find NBER working papers with cutting-edge identification strategies.
