...
5. **Benchmark correlation:** If replicating a known factor (e.g., FF HML), correlate with the published series. Expect r > 0.8 for value-weighted decile sorts. Use `search_papers` to locate the original paper defining the factor and its canonical construction details (e.g., "Fama French 1993 HML value factor"). Use `get_paper_details` to confirm the exact sorting variable, breakpoint convention, and weighting scheme.
...
7. **Literature novelty check:** Before publishing a new factor, use `search_papers` to verify the signal has not already been documented in the anomaly zoo. Search for both the variable name and the economic mechanism. Use `top_cited_articles` for the relevant subfield to identify canonical references that must be cited and benchmarked against.
