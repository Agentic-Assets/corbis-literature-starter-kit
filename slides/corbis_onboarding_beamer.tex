...
\begin{itemize}
  ...
  \item An MCP API key for key-based authentication
...
\begin{block}{Step 1: Use your Corbis account}
OAuth sign-in works without an MCP API key. The repository's MCP configuration contains only the Corbis server URL.
\end{block}
\begin{block}{Step 2: Connect Claude Code}
{\scriptsize\ttfamily
cd corbis-literature-starter-kit\\
claude
}
\end{block}
...
\begin{exampleblock}{Optional key-based setup}
If your client cannot use OAuth, create an MCP API key in Corbis Settings and configure a private Authorization header. Never place a key in a URL or a committed file.
\end{exampleblock}
\end{columns}
\end{frame}
...
\begin{itemize}
  ...
  \item Add the MCP server URL, then run \texttt{codex mcp login corbis}
  \item Verify the connection with \texttt{codex mcp list}
...
\begin{itemize}
  ...
  \item Paste the MCP endpoint URL and complete OAuth sign-in
