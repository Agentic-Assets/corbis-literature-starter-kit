...
- **Scratch directory:** `~/scratch` (symlink -- see setup instructions)
...
> **Note:** Direct local `psql service=wrds` is always preferred over SSH-tunneled psql. Use the `wrds-psql` skill for local queries. The SSH SQL examples below are a fallback for when local psql is not available.

Use `psql` directly with `~/.pgpass` for automatic authentication:
```bash
# Single query
psql service=wrds -c "SELECT * FROM crsp.dsf_v2 WHERE dlycaldt > '2025-01-01' LIMIT 10;"

# Output to CSV
psql service=wrds -c "COPY (SELECT * FROM optionm.securd LIMIT 100) TO STDOUT WITH CSV HEADER" > output.csv
```

**Via SSH (fallback):**
```bash
ssh wrds "psql -h wrds-pgdata.wharton.upenn.edu -d wrds -c 'SELECT * FROM crsp.dsf_v2 LIMIT 10;'"
```
